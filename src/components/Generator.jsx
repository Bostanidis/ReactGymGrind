...
export default function Generator(props) {
  ...
  return (
    <SectionWrapper id={"generate"} header={"generate your workout"} title={["It'\s", "Gym",
        "o\'clock"]}>
        <Header index={"01"} title={"Pick the structure"} description={"Select the workout you wish to endure"}></Header>
            ...
            {Object.keys(WORKOUTS).map((type, typeIndex) => {
              ...
              return (
                <button onClick={() => {
                  setMuscles([])
                  setPoison(type)
                }} className={"bg-slate-950 border px-4 duration-200 hover:border-blue-600 border-blue-400 py-3 rounded-lg " + (type === poison ? " border-blue-600 " : " border-blue-400 ")} key={typeIndex}>
        ...
        <Header index={"03"} title={"Growth type"} description={"Select your ultimate objective."}></Header>
          <div className='grid sm:grid-cols-3 grid-cols-1 gap-4'>
            {Object.keys(SCHEMES).map((scheme, schemeIndex) => {
              ...
              return (
                <button onClick={() => {
                  setGoal(scheme)
                }} className={"bg-slate-950 border duration-200 hover:border-blue-600 border-blue-400 py-3 px-4 rounded-lg " + (scheme === goal ? " border-blue-600 " : " border-blue-400 ")} key={schemeIndex}>
