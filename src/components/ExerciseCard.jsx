    ...
    return (
        ...
        bg-slate-950 sm:flex-wrap'>
            <div className='flex flex-col sm:flex-row sm:items-center sm:flex-wrap gap-x-4'>
                ...
                <h2 className='capitalize whitespace-nowrap truncate max-w-full text-lg sm:text-2xl
                md:text-3xl flex-1 sm:text-center'>{exercise.name.replaceAll("_", " ")}</h2>
                <p className='text-base text-slate-400 capitalize'>{exercise.type}</p>
            </div>
            <div className='flex flex-col'>
                <h3 className='text-slate-400 text-base text-'>Muscle Groups</h3>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2'>
                {["reps", "rest", "tempo"].map(info => {
                    return (
                        <div key = {info} className='flex flex-col p-2 rounded border-[1.5px] border-solid border-slate-900 w-full'>
                            <h3 className='text-slate-400'>{info === "reps" ? `${exercise.unit}` : info}</h3>
                ...
                <button onClick={handleSetIncrement} className='flex flex-col p-2 rounded border-[1.5px] border-solid border-blue-900 hover:border-blue-600 w-full duration-200'>
                    <h3 className='text-slate-400 text-sm capitalize'>Sets Complited</h3>
